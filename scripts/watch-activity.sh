...
if [ $# -eq 0 ]; then
  echo "Usage: make watch-activity tournamentId=<id> (e.g., make watch-activity tournamentId=12)"
  exit 1
fi
...
if [ -z "$TOURNAMENT_ID" ] || ! [[ "$TOURNAMENT_ID" =~ ^[0-9]+$ ]]; then
  echo "Invalid tournamentId: must be a positive integer. Usage: make watch-activity tournamentId=<id>"
  exit 1
fi
